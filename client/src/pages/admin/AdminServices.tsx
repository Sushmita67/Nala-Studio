    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          className={cn(
            'rounded-full px-3 py-1.5 text-sm font-medium uppercase tracking-[0.1em]',
            categoryFilter === 'all'
          ...
          {grouped.map((group) => (
            <section key={group.value} className="overflow-hidden rounded-xl border border-nala-border bg-nala-ivory">
              <div className="flex items-center justify-between border-b border-nala-border bg-nala-soft px-4 py-3">
                ...
                <span className="text-sm uppercase tracking-[0.12em] text-nala-muted">
                  {group.items.length} services
                </span>
              </div>
              <div className="overflow-x-auto">
                <table className="min-w-full text-left text-sm">
                  <thead className="text-sm uppercase tracking-[0.12em] text-nala-muted">
                    <tr>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="line-clamp-1 text-sm text-nala-muted">{s.description}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-sm">{formatPrice(s.price, s.priceLabel)}</td>
                        <td className="px-4 py-3 text-sm">
                          {formatDuration(s.duration, s.durationLabel)}
                        </td>
                        <td className="px-4 py-3 text-sm">{s.active ? 'Yes' : 'No'}</td>
                        <td className="px-4 py-3">
                          <div className="flex justify-end gap-2">
                            <button
                              type="button"
                              className="inline-flex items-center gap-1 rounded-full border border-nala-border bg-white px-2.5 py-1.5 text-sm font-medium uppercase tracking-[0.08em] text-nala-charcoal transition hover:border-nala-charcoal"
                            ...
                            <button
                              type="button"
                              className="inline-flex items-center gap-1 rounded-full border border-red-200 bg-red-50 px-2.5 py-1.5 text-sm font-medium uppercase tracking-[0.08em] text-red-800 transition hover:border-red-300"
        ...
        <div className="overflow-hidden rounded-xl border border-nala-border bg-nala-ivory">
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between border-t border-nala-border px-4 py-3">
              <button
                type="button"
                className="inline-flex items-center gap-1 rounded-full border border-nala-border px-3 py-1.5 text-sm uppercase tracking-[0.08em] disabled:opacity-40"
                disabled={page <= 1}
