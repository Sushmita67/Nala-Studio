import type {
  ClassSession,
  Course,
  Enrollment,
  GalleryImage,
  OwnerUser,
  Service,
  SiteContent,
  SiteSettings,
  Staff,
  Student,
  StudioData,
  Testimonial,
  WorkingHours,
} from '../types';
import {
  BRAND,
  DEFAULT_ADMIN_EMAIL,
  DEFAULT_ADMIN_PASSWORD,
  DEFAULT_FORMSPREE_FORM_ID,
} from '../config';

import heroSlide0 from '../assets/hero/hero-1.jpg';
import aboutImg from '../assets/images/nss1.jpg';

import nail1 from '../assets/images/Nail1.jpg';
import nails2 from '../assets/images/Nails2.jpg';
import nails3 from '../assets/images/Nails3.jpg';
import nails5 from '../assets/images/Nails5.jpg';
import nails8 from '../assets/images/Nails8.jpg';
import nails11 from '../assets/images/Nails11.jpg';
import nails13 from '../assets/images/Nails13.jpg';
import nails15 from '../assets/images/Nails15.jpg';
import nails19 from '../assets/images/Nails19.jpg';
import nails20 from '../assets/images/Nails20.jpg';
import nails21 from '../assets/images/Nails21.jpg';
import nails24 from '../assets/images/Nails24.jpg';
import nails26 from '../assets/images/Nails26.jpg';
import nails30 from '../assets/images/Nails30.jpg';
import nails35 from '../assets/images/Nails35.jpg';
import nails42 from '../assets/images/Nails42.jpg';
import nails52 from '../assets/images/Nails52.jpg';

import lash1 from '../assets/images/Lash1.jpg';
import lash2 from '../assets/images/Lash2.jpg';
import lash3 from '../assets/images/Lash3.jpg';
import lash4 from '../assets/images/Lash4.jpg';

import makeup1 from '../assets/images/Makeup1.jpg';
import makeup2 from '../assets/images/Makeup2.jpg';
import makeup3 from '../assets/images/Makeup3.jpg';
import makeup5 from '../assets/images/Makeup5.jpg';
import makeup9 from '../assets/images/Makeup9.jpg';
import makeup12 from '../assets/images/Makeup12.jpg';

import studio1 from '../assets/images/Studio1.jpg';
import studio2 from '../assets/images/Studio2.jpg';
import studio3 from '../assets/images/Studio3.jpg';
import studio6 from '../assets/images/Studio6.jpg';
import nss2 from '../assets/images/nss2.jpg';
import nss3 from '../assets/images/nss3.jpg';
import nss4 from '../assets/images/nss4.jpg';
import nss5 from '../assets/images/nss5.jpg';
import ns30 from '../assets/images/ns30.jpg';

const weekdayHours = (start = '10:00', end = '19:00'): WorkingHours[] =>
  [0, 1, 2, 3, 4, 5, 6].map((day) => ({
    day,
    startTime: day === 6 ? '11:00' : start,
    endTime: day === 6 ? '18:00' : end,
    isAvailable: day !== 0, // closed Sunday by default; adjust per staff
  }));

/**
 * Seed data layer.
 * PUBLIC UI RULE: `price` / `priceLabel` on services & courses must never be rendered
 * on public-facing routes. Prefer "Contact us for pricing" CTAs only.
 */
export const seedServices: Service[] = [
  // EYELASH EXTENSIONS
  {
    id: 'svc-lash-lift',
    name: 'Lash Lift',
    description: 'Lift and curl your natural lashes for an open-eye finish.',
    price: null,
    duration: 60,
    category: 'lashes',
    image: lash1,
    popular: true,
    active: true,
    order: 1,
    staffIds: ['staff-sushmita', 'staff-alisha'],
  },
  {
    id: 'svc-classic-set',
    name: 'Classic Set',
    description: 'One-to-one extensions for refined everyday length.',
    price: null,
    duration: 120,
    category: 'lashes',
    image: lash2,
    popular: true,
    active: true,
    order: 2,
    staffIds: ['staff-sushmita', 'staff-alisha'],
  },
  {
    id: 'svc-natural-volume',
    name: 'Natural Volume Set',
    description: 'Soft multi-lash fans for gentle everyday volume.',
    price: null,
    duration: 130,
    category: 'lashes',
    image: lash3,
    popular: true,
    active: true,
    order: 3,
    staffIds: ['staff-sushmita', 'staff-alisha'],
  },
  {
    id: 'svc-manga-lashes',
    name: 'Manga Lashes',
    description: 'Spiky, doll-inspired texture with graphic definition.',
    price: null,
    duration: 140,
    category: 'lashes',
    image: lash4,
    active: true,
    order: 4,
    staffIds: ['staff-sushmita', 'staff-alisha'],
  },
  {
    id: 'svc-mid-russian',
    name: 'Mid Russian Volume',
    description: 'Balanced Russian volume — fuller than classic, still wearable.',
    price: null,
    duration: 150,
    category: 'lashes',
    image: lash3,
    active: true,
    order: 5,
    staffIds: ['staff-sushmita', 'staff-alisha'],
  },
  {
    id: 'svc-full-russian',
    name: 'Full Russian Volume',
    description: 'Dense Russian volume for a bold, glamorous lash line.',
    price: null,
    duration: 160,
    category: 'lashes',
    image: lash4,
    popular: true,
    active: true,
    order: 6,
    staffIds: ['staff-sushmita', 'staff-alisha'],
  },
  {
    id: 'svc-wet-lash',
    name: 'Wet Lash',
    description: 'Glossy wet-look clusters for a fresh, dewy effect.',
    price: null,
    duration: 130,
    category: 'lashes',
    image: lash2,
    active: true,
    order: 7,
    staffIds: ['staff-sushmita', 'staff-alisha'],
  },
  {
    id: 'svc-light-wispy',
    name: 'Light Wispy',
    description: 'Feathery wisps with soft texture and airy movement.',
    price: null,
    duration: 135,
    category: 'lashes',
    image: lash1,
    active: true,
    order: 8,
    staffIds: ['staff-sushmita', 'staff-alisha'],
  },
  {
    id: 'svc-full-wispy',
    name: 'Full Wispy',
    description: 'Fuller wispy styling with layered texture and lift.',
    price: null,
    duration: 145,
    category: 'lashes',
    image: lash3,
    active: true,
    order: 9,
    staffIds: ['staff-sushmita', 'staff-alisha'],
  },
  {
    id: 'svc-mega-volume',
    name: 'Mega Volume',
    description: 'Maximum volume fans for dramatic lash impact.',
    price: null,
    duration: 170,
    category: 'lashes',
    image: lash4,
    active: true,
    order: 10,
    staffIds: ['staff-sushmita', 'staff-alisha'],
  },

  // GEL NAILS & EXTENSIONS
  {
    id: 'svc-overlay',
    name: 'Overlay',
    description: 'Strengthening gel overlay on natural nails.',
    price: null,
    duration: 75,
    category: 'gel-nails',
    image: nails11,
    popular: true,
    active: true,
    order: 11,
    staffIds: ['staff-alisha', 'staff-ashmita'],
  },
  {
    id: 'svc-gel-extensions',
    name: 'Gel Nail Extensions',
    description: 'Custom-length gel extensions shaped to you.',
    price: null,
    duration: 120,
    category: 'gel-nails',
    image: nails20,
    popular: true,
    active: true,
    order: 12,
    staffIds: ['staff-alisha', 'staff-ashmita'],
  },
  {
    id: 'svc-nails-remove',
    name: 'Nails Remove',
    description: 'Careful gel or extension removal that protects the nail.',
    price: null,
    duration: 45,
    category: 'gel-nails',
    image: nails3,
    active: true,
    order: 13,
    staffIds: ['staff-alisha', 'staff-ashmita'],
  },
  {
    id: 'svc-toe-nails',
    name: 'Toe Nails',
    description: 'Clean, polished gel colour for toes.',
    price: null,
    duration: 45,
    category: 'gel-nails',
    image: nails30,
    active: true,
    order: 14,
    staffIds: ['staff-alisha', 'staff-ashmita'],
  },
  {
    id: 'svc-toe-extensions',
    name: 'Toe Nail Extensions',
    description: 'Toe extensions for length and a refined finish.',
    price: null,
    duration: 75,
    category: 'gel-nails',
    image: nails30,
    active: true,
    order: 15,
    staffIds: ['staff-alisha', 'staff-ashmita'],
  },

  // PEDICURE
  {
    id: 'svc-dry-pedicure',
    name: 'Dry Pedicure',
    description: 'Clean dry pedicure with shaping and polish.',
    price: null,
    duration: 45,
    category: 'pedicure',
    image: nails30,
    active: true,
    order: 16,
    staffIds: ['staff-alisha', 'staff-ashmita'],
  },
  {
    id: 'svc-spa-pedicure',
    name: 'Spa Pedicure',
    description: 'Spa pedicure with soak, care and a polished finish.',
    price: null,
    duration: 60,
    category: 'pedicure',
    image: nails15,
    popular: true,
    active: true,
    order: 17,
    staffIds: ['staff-alisha', 'staff-ashmita'],
  },

  // MANICURE
  {
    id: 'svc-manicure',
    name: 'Manicure',
    description: 'Classic manicure with shaping, cuticle care and polish.',
    price: null,
    duration: 45,
    category: 'manicure',
    image: nail1,
    popular: true,
    active: true,
    order: 18,
    staffIds: ['staff-alisha', 'staff-ashmita'],
  },
  {
    id: 'svc-spa-manicure',
    name: 'Spa Manicure',
    description: 'Spa manicure with soak, massage and a refined finish.',
    price: null,
    duration: 60,
    category: 'manicure',
    image: nails2,
    popular: true,
    active: true,
    order: 19,
    staffIds: ['staff-alisha', 'staff-ashmita'],
  },

  // COURSES
  {
    id: 'svc-masters-nail',
    name: 'Masters Nail Course',
    description: 'Professional nail training with working NALA artists.',
    price: null,
    duration: null,
    durationLabel: 'Enquire',
    category: 'courses',
    image: nails24,
    active: true,
    order: 20,
    staffIds: ['staff-alisha'],
  },
  {
    id: 'svc-masters-lash',
    name: 'Masters Lash Course',
    description: 'Professional lash training for aspiring artists.',
    price: null,
    duration: null,
    durationLabel: 'Enquire',
    category: 'courses',
    image: lash3,
    active: true,
    order: 21,
    staffIds: ['staff-sushmita', 'staff-alisha'],
  },
  {
    id: 'svc-makeup-class',
    name: 'Makeup Class',
    description: 'Hands-on makeup class covering technique and finish.',
    price: null,
    duration: null,
    durationLabel: 'Enquire',
    category: 'courses',
    image: makeup1,
    active: true,
    order: 22,
    staffIds: ['staff-sushmita'],
  },
];

export const seedGallery: GalleryImage[] = [
  { id: 'gal-1', src: studio1, caption: 'NALA Studio interior', category: 'studios', featured: true, order: 1 },
  { id: 'gal-2', src: nails26, caption: 'Soft pink nail extensions', category: 'nails', featured: true, order: 2 },
  { id: 'gal-3', src: nails21, caption: 'Almond set with detail', category: 'nails', featured: true, order: 3 },
  { id: 'gal-4', src: nails20, caption: 'Long glam tip set', category: 'nails', featured: false, order: 4 },
  { id: 'gal-5', src: nails42, caption: 'Statement nail art', category: 'nails', featured: false, order: 5 },
  { id: 'gal-6', src: nails52, caption: 'Classic polish finish', category: 'nails', featured: false, order: 6 },
  { id: 'gal-7', src: nails13, caption: 'Soft glam almond nails', category: 'nails', featured: true, order: 7 },
  { id: 'gal-8', src: nails35, caption: 'Fine art detail', category: 'nails', featured: false, order: 8 },
  { id: 'gal-9', src: nails2, caption: 'Gel manicure finish', category: 'mani-pedi', featured: false, order: 9 },
  { id: 'gal-10', src: lash3, caption: 'Natural lash extensions', category: 'lashes', featured: true, order: 10 },
  { id: 'gal-11', src: makeup2, caption: 'Soft glam makeup', category: 'makeup', featured: true, order: 11 },
  { id: 'gal-12', src: nails30, caption: 'Pedicure detail', category: 'mani-pedi', featured: false, order: 12 },
  { id: 'gal-13', src: studio2, caption: 'Studio atmosphere', category: 'studios', featured: false, order: 13 },
  { id: 'gal-14', src: studio3, caption: 'Treatment room', category: 'studios', featured: false, order: 14 },
  { id: 'gal-15', src: nss4, caption: 'Class demonstration', category: 'class', featured: true, order: 15 },
  { id: 'gal-16', src: nss5, caption: 'Beauty education session', category: 'class', featured: false, order: 16 },
  { id: 'gal-17', src: lash4, caption: 'Volume lash set', category: 'lashes', featured: false, order: 17 },
  { id: 'gal-18', src: makeup3, caption: 'Bridal makeup look', category: 'makeup', featured: false, order: 18 },
  { id: 'gal-19', src: makeup9, caption: 'Evening glam', category: 'makeup', featured: false, order: 19 },
  { id: 'gal-20', src: makeup12, caption: 'Editorial makeup', category: 'makeup', featured: false, order: 20 },
  { id: 'gal-21', src: studio6, caption: 'Studio detail', category: 'studios', featured: false, order: 21 },
  { id: 'gal-22', src: nss3, caption: 'Course work in progress', category: 'class', featured: false, order: 22 },
  { id: 'gal-23', src: ns30, caption: 'Hands at work', category: 'class', featured: false, order: 23 },
  { id: 'gal-24', src: nss2, caption: 'Calm studio light', category: 'studios', featured: false, order: 24 },
];

export const seedCourses: Course[] = [
  {
    id: 'course-nails',
    name: 'Masters Nail Course',
    description:
      'Master nail preparation, gel application, extensions, shaping and finish with working NALA artists.',
    duration: 'Flexible',
    price: 'On request',
    image: nails24,
    curriculum: [
      'Nail anatomy, hygiene & sanitation',
      'Manicure & cuticle work',
      'Gel application and finishing',
      'Extensions & structure',
      'Shaping and refinement',
      'Client care & studio practice',
    ],
    enrollmentInfo: 'Contact the studio to discuss schedule and enrollment.',
    active: true,
    order: 1,
  },
  {
    id: 'course-lashes',
    name: 'Masters Lash Course',
    description:
      'Hands-on lash extension training covering mapping, isolation, application and aftercare.',
    duration: 'Flexible',
    price: 'On request',
    image: lash1,
    curriculum: [
      'Lash health & safety',
      'Lash mapping and styling',
      'Isolation and application',
      'Retention and aftercare',
      'Removal techniques',
      'Building a lash clientele',
    ],
    enrollmentInfo: 'Contact the studio to discuss schedule and enrollment.',
    active: true,
    order: 2,
  },
  {
    id: 'course-makeup',
    name: 'Makeup Class',
    description:
      'A practical makeup class covering base work, eye looks and occasion-ready finishing.',
    duration: 'Flexible',
    price: 'On request',
    image: makeup1,
    curriculum: [
      'Skin prep & base',
      'Eye definition & lashes',
      'Contour and colour',
      'Occasion & bridal looks',
      'Hygiene and kit setup',
    ],
    enrollmentInfo: 'Contact the studio to discuss schedule and enrollment.',
    active: true,
    order: 3,
  },
];

export const seedStaff: Staff[] = [
  {
    id: 'staff-alisha',
    name: 'Alisha',
    role: 'nail_tech',
    title: 'Lead Nail & Lash Artist',
    bio: 'Specialises in extensions, gel and natural-looking lash sets with a meticulous finish.',
    specialties: ['Gel Extensions', 'Manicure', 'Lash Sets'],
    image: studio1,
    serviceIds: [
      'svc-lash-lift',
      'svc-classic-set',
      'svc-natural-volume',
      'svc-manga-lashes',
      'svc-mid-russian',
      'svc-full-russian',
      'svc-wet-lash',
      'svc-light-wispy',
      'svc-full-wispy',
      'svc-mega-volume',
      'svc-overlay',
      'svc-gel-extensions',
      'svc-nails-remove',
      'svc-toe-nails',
      'svc-toe-extensions',
      'svc-dry-pedicure',
      'svc-spa-pedicure',
      'svc-manicure',
      'svc-spa-manicure',
      'svc-masters-nail',
      'svc-masters-lash',
    ],
    workingHours: weekdayHours().map((h) => ({ ...h, isAvailable: h.day !== 0 })),
    active: true,
    order: 1,
  },
  {
    id: 'staff-ashmita',
    name: 'Ashmita Bishwakarma',
    role: 'nail_tech',
    title: 'Nail Artist',
    bio: 'Known for clean structure, soft colour and detailed nail work.',
    specialties: ['Gel Overlay', 'Manicure', 'Pedicure'],
    image: nails26,
    serviceIds: [
      'svc-overlay',
      'svc-gel-extensions',
      'svc-nails-remove',
      'svc-toe-nails',
      'svc-toe-extensions',
      'svc-dry-pedicure',
      'svc-spa-pedicure',
      'svc-manicure',
      'svc-spa-manicure',
    ],
    workingHours: weekdayHours().map((h) => ({
      ...h,
      isAvailable: h.day !== 0 && h.day !== 6,
    })),
    active: true,
    order: 2,
  },
  {
    id: 'staff-sushmita',
    name: 'Sushmita Bishwakarma',
    role: 'lash_tech',
    title: 'Lashes & Makeup Artist',
    bio: 'Creates soft glam lashes and occasion makeup with a light, natural touch.',
    specialties: ['Volume Lashes', 'Wispy Sets', 'Makeup'],
    image: makeup2,
    serviceIds: [
      'svc-lash-lift',
      'svc-classic-set',
      'svc-natural-volume',
      'svc-manga-lashes',
      'svc-mid-russian',
      'svc-full-russian',
      'svc-wet-lash',
      'svc-light-wispy',
      'svc-full-wispy',
      'svc-mega-volume',
      'svc-masters-lash',
      'svc-makeup-class',
    ],
    workingHours: weekdayHours().map((h) => ({ ...h, isAvailable: h.day !== 0 })),
    active: true,
    order: 3,
  },
];

export const seedStudents: Student[] = [
  {
    id: 'stu-1',
    name: 'Priya Shrestha',
    email: 'priya.shrestha@example.com',
    phone: '9801112233',
    notes: 'Interested in nail extensions career path.',
    enrolledAt: '2026-01-15T00:00:00.000Z',
    active: true,
  },
  {
    id: 'stu-2',
    name: 'Anisha Gurung',
    email: 'anisha.gurung@example.com',
    phone: '9802223344',
    notes: 'Lash course candidate.',
    enrolledAt: '2026-02-01T00:00:00.000Z',
    active: true,
  },
];

export const seedClasses: ClassSession[] = [
  {
    id: 'class-nails-spring',
    courseId: 'course-nails',
    name: 'Nail Course — Spring Cohort',
    instructorId: 'staff-alisha',
    startDate: '2026-04-01',
    endDate: '2026-05-15',
    capacity: 8,
    location: 'NALA Studio, Phulbari',
    active: true,
  },
  {
    id: 'class-lashes-spring',
    courseId: 'course-lashes',
    name: 'Lash Course — Spring Cohort',
    instructorId: 'staff-sushmita',
    startDate: '2026-04-10',
    endDate: '2026-05-20',
    capacity: 6,
    location: 'NALA Studio, Phulbari',
    active: true,
  },
];

export const seedEnrollments: Enrollment[] = [
  {
    id: 'enr-1',
    studentId: 'stu-1',
    classId: 'class-nails-spring',
    courseId: 'course-nails',
    status: 'active',
    enrolledAt: '2026-03-01T00:00:00.000Z',
    progress: 40,
    notes: '',
  },
  {
    id: 'enr-2',
    studentId: 'stu-2',
    classId: 'class-lashes-spring',
    courseId: 'course-lashes',
    status: 'active',
    enrolledAt: '2026-03-05T00:00:00.000Z',
    progress: 20,
    notes: '',
  },
];

export const seedOwner: OwnerUser = {
  id: 'owner-1',
  email: DEFAULT_ADMIN_EMAIL,
  password: DEFAULT_ADMIN_PASSWORD,
  name: 'NALA Owner',
  role: 'owner',
};

export const seedTestimonials: Testimonial[] = [
  {
    id: 'rev-1',
    name: 'Sushmita Biswakarma',
    text: 'Lovely studio and beautiful nail work — the detail and finish were exactly what I wanted.',
    rating: 5,
    date: '9 months ago',
    source: 'Google',
    featured: true,
    visible: true,
  },
  {
    id: 'rev-2',
    name: 'Google Reviewer',
    text: 'Clean, calm space and the lashes turned out so natural. Will be back.',
    rating: 5,
    date: 'Google review',
    source: 'Google',
    featured: true,
    visible: true,
  },
  {
    id: 'rev-3',
    name: 'Google Reviewer',
    text: 'Great service and really talented artists. Happy with my gel manicure.',
    rating: 5,
    date: 'Google review',
    source: 'Google',
    featured: true,
    visible: true,
  },
];

export const seedContent: SiteContent = {
  heroTitle: 'Beauty, crafted for you',
  heroSubtitle: 'Premium nails, lashes & beauty treatments in Kathmandu.',
  heroImage: heroSlide0,
  heroSlides: [
    { id: 'slide-1', src: heroSlide0, alt: 'NALA Studio beauty atmosphere', order: 1, active: true },
    { id: 'slide-2', src: studio1, alt: 'Inside the NALA Studio space', order: 2, active: true },
    { id: 'slide-3', src: nails24, alt: 'Nail art and manicure work', order: 3, active: true },
    { id: 'slide-4', src: lash3, alt: 'Lash treatment detail', order: 4, active: true },
    { id: 'slide-5', src: makeup2, alt: 'Makeup artistry at NALA', order: 5, active: true },
    { id: 'slide-6', src: studio3, alt: 'Calm studio lighting', order: 6, active: true },
    { id: 'slide-7', src: aboutImg, alt: 'NALA Studio interior', order: 7, active: true },
  ],
  heroCtaPrimary: 'Book an appointment',
  heroCtaSecondary: 'Explore services',
  aboutHeading: 'Where Beauty Meets Detail',
  aboutDescription:
    'NALA Studio is a modern beauty destination in Kathmandu specializing in nails, lashes, makeup and beauty education. Every service is personal, polished and carefully crafted — from a simple polish change to a full set of extensions, lashes or bridal makeup.',
  aboutImage: aboutImg,
  instagramHandle: '@nala.studio__',
  instagramUrl: BRAND.instagram,
  facebookUrl: BRAND.facebook,
  phone: BRAND.phone,
  addressLine1: 'Triple Seven Complex',
  addressLine2: 'Phulbari, Kathmandu, Nepal',
  mapsUrl: BRAND.mapsUrl,
  hoursWeekday: 'Sunday – Friday: 10:00 AM – 7:00 PM',
  hoursSaturday: 'Saturday: 11:00 AM – 6:00 PM',
  googleRating: '4.7',
  googleReviewCount: '27',
};

export const seedSettings: SiteSettings = {
  formspreeFormId: DEFAULT_FORMSPREE_FORM_ID,
  adminPassword: DEFAULT_ADMIN_PASSWORD,
  adminEmail: DEFAULT_ADMIN_EMAIL,
  studioName: BRAND.name,
  certificatePrefix: 'NALA',
  bookingTimeSlots: ['10:00', '11:00', '12:00', '13:00', '14:00', '15:00', '16:00', '17:00', '18:00'],
};

export function createSeedData(): StudioData {
  return {
    services: seedServices,
    gallery: seedGallery,
    courses: seedCourses,
    testimonials: seedTestimonials,
    certificates: [],
    bookings: [],
    media: seedGallery.map((g, i) => ({
      id: `media-${i + 1}`,
      name: g.caption,
      src: g.src,
      createdAt: new Date().toISOString(),
    })),
    staff: seedStaff,
    students: seedStudents,
    classes: seedClasses,
    enrollments: seedEnrollments,
    owner: seedOwner,
    content: seedContent,
    settings: seedSettings,
    certificateCounter: 0,
  };
}
